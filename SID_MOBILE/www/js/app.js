...
function trocaPublicacao(dados) {
  ...
  const intervalo = setInterval(function () {
    ...
    $$("#img").append("<img id='imgDivulgacao' src='data:image/png;base64," + dados[posicao]['imagem'] + "'alt='Imagem divulgacao' ></img>");
    $$("#legenda").append("<div id='legendaDivulgacao'>" + dados[posicao]['bd']['legenda'] + "</div>");

    url = dados[posicao]['bd']['linkqr'];
    $$("#qrcode").append("<button class'col button' id='qrCodeDivulgacao'></button>");
    if (dados[posicao]['comentarios'] != null) {
      var randon = Math.floor(Math.random() * (dados[posicao]['comentarios']).length);
      $$("#imgPerfil").append("<img id='imgDivulgacaoPerfil' src='" + dados[posicao]['comentarios'][randon]['urlFoto'] + "' alt='Imagem divulgacao' ></img>");
      ...
      if ($$("#select_turmas").val() != "Seleciona a Turma") {
        if ($$("#mensagem").val() != "") {
          app.request.post('http://200.130.152.84:6670/mobile/menssagens/',
            { matricula: matricula, id: $$("#select_turmas").val(), mensagem: $$("#mensagem").val() },
            function (data) {
              app.dialog.alert("Inserido com sucesso!");
              $$("#mensagem").val("");
            });
        } else {
          app.dialog.alert("Digite uma mensagem!");
        }
      } else {
    ...
    app.request.json('http://200.130.152.84:6670/mobile', function (data) {
      for (var i = 0; i < data.length; i++) {
        $$("#lista").append(
          ...
          "<br/>Turma: "
          + data[i]['id_turma'] +
          "</div><div class='card-content card-content-padding'> <b>Mensagem</b><br/>"
