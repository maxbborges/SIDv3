...
\section*{Introdução}
	...
	\danielobs{Quem disse isso? Seria interessante embasar este argumento por meio de citação de uma referência da área.}
	
	A divulgação de forma estática e de formas mais tradicionais (revistas e jornais) já não são as formas mais eficientes de se expor um conteúdo ou propaganda\daniel{, para}{. Para} \cite{escobar2007} novas tecnologias de comunicação colocaram a interatividade em evidencia, então,  a utilização de novas ferramentas mais interativas com seu receptor, torna a leitura menos monótona e é possível conseguir uma maior atenção do espectador, além de \daniel{se conseguir}{} atingir um maior numero de pessoas de forma mais consistente.
	...
	\danielobs{Internet é maiúsculo.}
...
\section*{Revisão da Literatura}
	...
	\danielobs{Hastag não é o termo correto. Hashtag}
	
	\danielobs{Defina o que é tela pública. Não está claro para o leitor. Fale algo no sentido de que é uma tela por assinatura do serviço.}
	...
	\danielobs{Não entendi esta comparação. O SID é gratuito.}
...
\section*{Proposta}
	...
	\danielobs{Conversando com  o Fontes, ele achou o escopo do projeto muito limitado. Sugiro aumentar o escopo. A proposta agora deve incluir: \begin{itemize}
			\item Integração com as redes sociais facebook.
			\item Emprego de técnicas de mineração de dados de modo a escolher os comentários mais adequados a serem publicados.
			\item Implementação de uma versão mobile.
			\item Suporte para uma integração ao SGA, possibilitando avisos personalizados para determinado aluno ou para determinada turma através de notificações mobile.
		\end{itemize}}
...
\section*{Metodologia}
	...
	Usando o SID como sistema base e com uma estrutura cliente-servidor, será implementado no sistema as interações com as redes sociais. As informações serão apresentadas em multiplataforma que são televisores, painéis, paginas web ou celulares, essas informações podem ser alteradas usando o servidor, um \textit{Raspberry Pi}, com o sistema instalado e conectado a internet. Após serem criadas ou modificadas, as publicadas poderão ser transmitidas e acessadas pelos clientes em distintas plataformas ao mesmo tempo.
...
\section*{Cronograma e Descrição Orçamentária do Projeto}
	\danielobs{Coloque um parágrafo no início desta seção. Você está apresentando o cronograma de uma maneira muito seca. Coloque algo do tipo `` O cronograma do projeto subdivide-se nestas etapas, que estão dispostas nos próximos meses da seguinte maneira: }
	...
	\item Melhorias gerais no código e no banco de dados do sistema usado como base.
	
	\item Melhorias da interação do sistema com Facebook.
	
	\item Implementação de outras possíveis redes sociais.
	
	\item Mineração de dados para recuperação de comentários.
	
	\item Implementação da moderação dos comentários que serão publicados.
	
	\item Porte do sistema para mobile.
...
\begin{table}[h!]
	...
	\begin{tabular}{c|c|c|c|c|c|c|c|c|c|c|}
		...
		\multicolumn{1}{|c|}{V}		& & & & &X&X& & & &\\ \hline
		\multicolumn{1}{|c|}{VI}	& & & & & &X&X& & &\\ \hline
		\multicolumn{1}{|c|}{VII}	& & & & & &X&X& & &\\ \hline
		\multicolumn{1}{|c|}{VIII}	& & & & & & &X&X&X&\\ \hline
		\multicolumn{1}{|c|}{IX}	& & & & & & & &X&X&X\\ \hline
	\end{tabular}
\end{table}

\danielobs{Eu duvido que iremos levar apenas dois meses para implementar o sistema. Coloque prazos mais factíveis.}
