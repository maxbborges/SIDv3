...
Quanto maior é a quantidade dedados, maior é a necessidade de o uso de um banco de dados. O uso de papéis para armazenar informações pode necessitar de uma grande quantidade de espaço físico e de um grande controle, para que nada se perda.
...
A Application Programming Interface (API) é um instrumento que visa facilitar o desenvolvimento e a vida dos programadores. 
...
\subsection{REST}
A Representational State Transfer (REST), são \textit{web services} que permitem que os clientes acessem e manipulem os dados que estão disponíveis para serem feito requisições. 
...
Com a arquitetura cliente-servidor, a REST é usadas para que os clientes possam realizar requisições de dados aos servidores, seguindo as regras e com uso dos métodos GET, POST, DELETE e PUT.  
...
\section{JSON}
\maxwell{podendo possuir os três tipos básicos de conteúdo: numérico, booleano e String, com conteúdo associativo ou não. O JSON é um modelo para armazenamento e transmissão de informações no formato texto e com capacidade de transmitir um grande volume de dados.}
